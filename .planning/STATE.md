...
- [Roadmap]: Delivery pipeline (Phase 1) and analytics (Phase 2) come first — nothing downstream is verifiable until shipping is safe, and the KPI is unmeasurable until PostHog is live.
...
- [Branding]: Analytics backend is self-hosted PostHog (2026-09-04), superseding the self-hosted Umami decision of 2026-08-30 — owning the data is still the reason to self-host, but PostHog costs a ClickHouse/Kafka/Redis stack where Umami was one container, and its cookie+autocapture configuration now requires a consent banner the page does not have
...
- [Phase 2] CONV-01 still open: the self-hosted PostHog instance does not exist. The page is fully instrumented and the loader sits commented in `<head>`; `.planning/RUNBOOK-posthog.md` is the remaining work. The two bracket tokens were removed from `PLACEHOLDERS` on 2026-09-04 so the site could ship without analytics — putting them back is step 3 of that runbook. CONV-03 (ratio readable without arithmetic) is satisfiable in PostHog via a funnel or formula insight, but that insight still has to be built. A consent banner is a new prerequisite, and is unbuilt.
